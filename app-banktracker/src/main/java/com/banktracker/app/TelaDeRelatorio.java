...
public class TelaDeRelatorio extends javax.swing.JFrame {
    ...
    private Integer vezesUsoAcima = 0;
    /**
    ...
    public void MetodoPrincipal() {
        ...
        cx.capturarHistorico();
        
        atualizarTela(cx);
        
        System.out.println("Finalizando processos...");
        cx.getFinalizador().FinalizarProcessos(cx.getHostname(), cx.getSistemaOperacional());
    }
    
    public void atualizarTela(CaixaEletronico cx) {
        atualizarDadosMaquinaTela(cx);
        atualizarDadosUtilizacaoTela(cx);   
    }
    
    public void atualizarDadosMaquinaTela(CaixaEletronico cx){
        Double frequencia = (cx.getCpuCaixa().doubleValue() / 1e+9);
        Double ramGB = cx.getRam().doubleValue() / 1e+9;
        Double hdTrunc = cx.getHd().doubleValue() / 1e+9;
        lblSistemaOperacional.setText(cx.getSistemaOperacional());
        lblCpuFisica.setText(cx.getNucleos().toString());
        lblCpuLogica.setText(cx.getThreads().toString());
        lblFrequencia.setText(String.format("%.2f GHz", frequencia));
        lblProcessador.setText(cx.getProcessador());
        lblRamTotal.setText(String.format("%.2f GB", ramGB));
        System.out.println(String.format("%.2f GB", ramGB));
        lblHd.setText(String.format("%.2f GB", hdTrunc));
        System.out.println(String.format("%.2f GB", hdTrunc));
        String listaDiscos = "";
    ...
    public void atualizarDadosUtilizacaoTela(CaixaEletronico cx) {
        Historico historico = cx.ultimoHistorico();
        Date date = new Date();
        ...
        Double usoCpu = historico.getUsoCpu();
        lblPorcentagemDeUso.setText(String.format("%.2f", usoCpu)+"%");
        if(usoCpu > 80){
           vezesUsoAcima++;
            System.out.println("O uso de cpu está acima do normal, "+ vezesUsoAcima);
        }else{
            vezesUsoAcima = 0;
            System.out.println("O uso de cpu está normal");
        }

        if(vezesUsoAcima == 3){
            SlackMensagens.enviarMensagem(String.format("O caixa %s está com o uso de CPU = %.2f%%", cx.getHostname(), usoCpu));
        }
       
        
        //Double porcentagemRam = (historico.getUsoRam() *100) / historico.getMemoria().getDisponivel();
        //System.out.println(porcentagemRam);
        
        System.out.println(historico.getUsoRam());
        ...
        Double ramEmUsoGB = (historico.getUsoRam().doubleValue() / 1e+9);
        lblRamEmUso.setText(String.format("%.2f GB", ramEmUsoGB));
        Double ramLivreGB = (looca.getMemoria().getDisponivel()/1e+9);
        lblRamLivre.setText(String.format("%.2f GB", ramLivreGB));
        
        Double discoLivreGB = historico.getDiscoLivre().doubleValue() / 1e+9;
        Double usoDiscoGB = (historico.getUsoDisco().doubleValue()/1e+9);
